...
export default function ListarMedicos({ navigation }) {
  ...
  const loadMedicos = async () => {
    try {
      setLoading(true);
      const medicosResult = await AuthService.getMedicosConEspecialidades(); // Método que debes crear
  ...
  const renderAccionesMedico = (medico) => {
    ...
    return (
      <View style={styles.accionesContainer}>
        {/* Horarios disponibles para pacientes */}
        {user?.role === 'paciente' && (
        ...
        <View style={styles.accionesMedico}>
          ...
          {/* Botón Editar - Solo admin y el mismo médico */}
    ...
    <View style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        <View style={styles.titleContainer}>
          ...
          {/* Solo admin puede agregar médicos */}
          {user?.role === 'admin' && (
            <TouchableOpacity 
